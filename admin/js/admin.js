    ...
    if (accounts.length === 0) {
      container.innerHTML = `<div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-2xl p-5 mb-4 text-sm text-slate-600 dark:text-slate-300">
        ...
        <ul class="list-disc ml-5 space-y-1 text-xs">
          <li><strong>Checking / savings</strong> — deposits = income, withdrawals &amp; bill pays = expenses (including credit card payments).</li>
          <li><strong>Credit card</strong> — purchases = expenses, statement payments &amp; credits = income. Paying from checking shows expense there and income on the card — they net out.</li>
        </ul></div>
  ...
  document.getElementById('addAccountBtn')?.addEventListener('click', () => {
    openModal('Add Account', `
      <form id="accountForm" class="space-y-4">
        ...
        <p class="text-xs text-slate-500">Use <strong>Credit card</strong> for card purchases (expense) and payments/credits (income). <strong>Checking</strong> for bank cash flow — CC payments are expenses here.</p>
        <div><label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Institution (optional)</label>
